...
const model = genAI.getGenerativeModel({
    ...
    generationConfig: {
        temperature: 0, // 창의성 제거, 지침 준수 극대화
        topP: 0.1,
    }
});
...
app.post('/api/analyze', async (req, res) => {
    try {
        ...
        // [최후의 수단] AI에게 줄바꿈과 템플릿 준수를 위해 '반말'과 '강력한 경고'를 섞어 명령합니다.
        const prompt = `
            너의 임무는 아래 [입력 데이터]를 사용하여 [최종 리포트 템플릿]의 빈칸을 채우는 것이다.
            
            [절대 규칙]
            1. 리포트의 시작은 반드시 ──────────────────────── 로 시작하라.
            2. 템플릿에 없는 설명, 분석 의견, Disclaimer 등은 단 한 글자도 적지 마라.
            3. 모든 케이스(1~6)를 빠짐없이 포함하라.
            4. 줄바꿈 규칙을 엄격히 준수하라.
